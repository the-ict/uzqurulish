...
class ComplianceService {
    async checkProject(projectId, projectParams) {
        // Real compliance check logic based on project parameters
        const rules = [
            { name: "Balandlik cheklovi", weight: 25, check: (params) => this.checkHeight(params) },
            { name: "Yong'in xavfsizligi", weight: 25, check: (params) => this.checkFireSafety(params) },
            { name: "Ekologik talablar", weight: 25, check: (params) => this.checkEnvironmental(params) },
            { name: "Sanitariya normalari", weight: 25, check: (params) => this.checkSanitary(params) },
        ];
        const results = [];
        let totalScore = 0;
        ...
        await models_1.default.Compliance.bulkCreate(results.map((result) => ({
            projectId: projectId,
    ...
    checkHeight(params) {
        ...
        const maxHeight = heightLimits[type] || 12;
        if (height <= maxHeight) {
    ...
    generateDetails(ruleName, projectParams) {
    ...
    async getComplianceByProjectId(projectId) {
        const complianceRecords = await models_1.default.Compliance.findAll({
            where: { projectId },
