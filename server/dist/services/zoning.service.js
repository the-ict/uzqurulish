...
class ZoningService {
    async getZoningByLocation(location) {
        const zoningData = await models_1.default.Zoning.findOne({
    ...
    async getAllZoneTypes() {
        // Try to get zone types from database first
        try {
            const zoneTypes = await models_1.default.Zoning.findAll({
                attributes: [
                    [models_1.default.sequelize.fn('DISTINCT', models_1.default.sequelize.col('zone_type')), 'zoneType']
                ],
                raw: true
            });
            if (zoneTypes && zoneTypes.length > 0) {
                ...
                for (const zone of zoneTypes) {
                    const sampleData = await models_1.default.Zoning.findOne({
    ...
    async getZoningMapData() {
        // Get real zoning data from database
        try {
            const zoningData = await models_1.default.Zoning.findAll();
    ...
    async seedZoningData() {
        ...
        for (const data of sampleData) {
            const [zoningRecord, created] = await models_1.default.Zoning.findOrCreate({
